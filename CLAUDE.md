...
- **`.github/workflows/release.yml`** — builds the Flatpak bundle (`GDLauncher.flatpak`) in the freedesktop 24.08 container and creates a GitHub Release named `GDLauncher <tag>` with the bundle attached. It releases in two cases: called as a reusable workflow with a `tag` input (`workflow_call`, from check-update), or a version tag (`*.*.*`) pushed directly (e.g. locally via `mise run update`, or to republish/replace a release by pushing the tag by hand). It also runs on `pull_request` for build-only validation. The tag/version is `inputs.tag || github.ref_name`.
